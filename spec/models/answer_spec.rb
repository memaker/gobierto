...
RSpec.describe Answer, type: :model do
  describe '.percentages_for_question' do
    it 'should return a hash with the percentages' do
      ...
      Answer.create({answer_text: 'Apropiado'}.merge(valencia_attributes))
      Answer.create({answer_text: 'Apropiado'}.merge(madrid_attributes))
      Answer.create({answer_text: 'Apropiado'}.merge(madrid_attributes))
      Answer.create({answer_text: 'Poco'}.merge(madrid_attributes))
      Answer.create({answer_text: 'Poco'}.merge(madrid_attributes.merge({code: 2})))

      percentages = Answer.percentages_for_question(2, {year: 2015, place_id: 28079, code: '1', area_name: 'economic', kind: 'G'})
